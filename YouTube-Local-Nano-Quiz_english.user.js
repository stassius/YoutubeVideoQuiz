...
// @updateURL    https://raw.githubusercontent.com/stassius/YoutubeVideoQuiz/main/YouTube-Local-Nano-Quiz_english.user.js
...
(() => {
    ...
    const QUIZ_RULES = `
Subject boundary:
- Test the video's subject matter: concepts, facts, causes, methods, calculations, and problem-solving.
- Do not test lesson logistics or classroom communication: teacher actions or advice, student behavior, questions during the lecture, microphones, chat, cameras, seminars, consultations, schedules, or homework arrangements.
- A logistical detail mentioned in the transcript is not a learning topic. Do not turn it into a question or true_false statement.
- Boundary test: if a candidate loses its meaning after removing “teacher”, “student”, “lecture”, or “seminar”, reject it and replace it with a subject-matter question.
...
{{chunk}}
        ...
        ANSWER_CHECK_PROMPT: `
Strictly check the student's answer for completeness and factual accuracy.

Return true only if the answer fully addresses this exact question, contains every required fact from at least one reference answer, and has no factual errors.
Return false if any required element is missing, an incorrect element is added, only a broader category is named, or the answer merely concerns the same topic.
For a requested list, require every item. Allow synonyms, rewording, and grammar mistakes only when the complete meaning is preserved.
Rule example: for reference “A, B, C”, both “A, B” and “A, B, D” are false.
...
Create a child-friendly educational quiz: {{requiredCount}} simple required questions and {{optionalCount}} harder optional questions.

Use suitable question types from:
{{questionTypes}}

${QUIZ_RULES}

Ignore ads, sponsorships, greetings, jokes, off-topic remarks, subscription requests, and other irrelevant fragments.

All questions, answer options, correct answers, and explanations must be in English.
...
`.trim(),
    ...
    function parseTrueFalseAnswer(value) {
        ...
        const falseTokens = ['false', 'no', 'incorrect', 'wrong'];
        const trueTokens = ['true', 'yes', 'correct', 'right'];

        if (
            falseTokens.some(token => tokens.has(token)) ||
            answer.includes('not true') ||
            answer.includes('does not correspond to reality')
        ) {
            return false;
        }
    ...
    function ensureMounted() {
        ...
        intro.textContent = 'Gemini Nano will generate questions about the video content.';
    ...
    function findShowTranscriptButton() {
        ...
        for (const scope of scopes) {
            for (const button of scope.querySelectorAll('button, tp-yt-paper-button')) {
                ...
                if (/transcript/i.test(text)) return button;
            }
        }
    ...
    async function openYouTubeTranscript() {
        ...
        if (!button) {
            throw new Error(
                'Could not find YouTube’s “Show transcript” button. ' +
    ...
    async function loadTranscript() {
        setStatus('Opening the YouTube transcript...');
    ...
    async function compactLongTranscript(transcript, initialSession, expectedEpoch) {
        ...
        try {
            for (let i = 0; i < chunks.length; i++) {
                ...
                setStatus(
                    `Preparing material: fragment ${i + 1}/${chunks.length}...`
                );
                ...
                try {
                    ...
                    setStatus(
                        `Preparing fragment ${i + 1}/${chunks.length} again...`
                    );
                    raw = await session.prompt(chunkPrompt);
                }
                ...
                if (!summary) {
                    throw new Error(`Gemini Nano could not condense fragment ${i + 1}.`);
                }

                summaries.push(`[Fragment ${i + 1}]\n${summary}`);
                session.destroy?.();
                session = null;
    ...
    async function createLanguageModel() {
        if (!('LanguageModel' in PAGE)) {
            throw new Error(
                'The Chrome LanguageModel API was not found. Built-in Gemini Nano ' +
                'is unavailable in this Chrome configuration.'
            );
        }

        // Deliberately do NOT request Translator API and do NOT force an
        // expected language here. The English transcript is passed directly.
        ...
        setStatus(
            availability === 'available'
                ? 'Gemini Nano is available. Reading the transcript directly...'
        ...
        return await PAGE.LanguageModel.create({
            monitor(monitor) {
                monitor.addEventListener('downloadprogress', event => {
                    setStatus(
                        `Downloading Gemini Nano... ${Math.round((event.loaded || 0) * 100)}%`
                    );
                });
            },
        });
    ...
    function buildPrompt({
        ...
        return CONFIG.PROMPT
            .replaceAll('{{playlistTitle}}', playlistTitle || 'not specified')
            .replaceAll('{{videoTitle}}', videoTitle || 'not specified')
            .replaceAll(
                '{{requiredCount}}',
...
This is a new batch. Do not repeat or paraphrase any existing question:
${existingQuestions}
`.trimEnd();
    }
    ...
    async function fitPromptToContext(session, transcript, schema, suffix = '') {
        ...
        while (true) {
            ...
            if (
                ...
                throw new Error(
                    `The transcript does not fit the Gemini Nano context (${used}/${session.contextWindow}).`
                );
            }

            working = nextWorking;
            setStatus(`Reducing the long transcript to ${working.length} characters...`);
        }
    }
    ...
    async function promptForQuiz(session, prompt, schema) {
        ...
        try {
            ...
            if (error?.name === 'NotSupportedError') {
                throw new Error(
                    'This Chrome/Gemini Nano build refused to process English text directly. ' +
                    'Translation is intentionally disabled in the script.'
                );
            }
            throw error;
        }
        ...
        try {
            ...
            throw new Error('Gemini Nano returned invalid JSON.');
        }
    ...
    async function generateQuizWithNano(transcript, session) {
        ...
        setStatus('Gemini Nano is generating questions...');
    ...
    function isRecoverableNanoError(error) {
        ...
        return error?.name === 'QuotaExceededError' ||
            ...
            /context|token|window|quota|output limits|truncated|invalid json|kErrorUnknown|unknown error/i.test(message);
    }
    ...
    async function generateMoreQuestionsWithNano() {
        ...
        if (!generationSession || !cachedTranscript) {
            throw new Error('Quiz context was not found. Create the main quiz first.');
        }
        ...
        setStatus('Gemini Nano is creating new questions in a clean session...');
        ...
        try {
            ...
            setStatus('Context refreshed. Gemini Nano is generating more questions...');
            ...
            group.forEach((rawQuestion, index) => {
                try {
                    ...
                    if (seenPrompts.has(promptKey)) {
                        throw new Error('the question exactly duplicates an existing question');
                    }
    ...
    function normalizeQuestion(raw, index) {
        ...
        if (!CONFIG.ENABLED_TYPES.includes(q.type)) {
            throw new Error(`Disabled question type: ${q.type || '(empty)'}`);
        }
        ...
        if (q.type === 'single_choice') {
            if (!q.correctAnswers.length) {
                throw new Error(`single_choice ${index + 1}: no correct answer.`);
            }
            ...
            if (q.options.length < 2) {
                throw new Error(`single_choice ${index + 1}: too few options.`);
            }
        }
        ...
        if (q.type === 'multiple_choice') {
            if (!q.correctAnswers.length) {
                throw new Error(`multiple_choice ${index + 1}: no correct answers.`);
            }
            ...
            if (q.options.length < 2) {
                throw new Error(`multiple_choice ${index + 1}: too few options.`);
            }
        }
        ...
        if (q.type === 'true_false') {
            ...
            if (!q.correctAnswers.length) {
                throw new Error(`true_false ${index + 1}: no correct answer.`);
            }

            const parsedAnswer = parseTrueFalseAnswer(q.correctAnswers[0]);
            if (parsedAnswer === null) {
                throw new Error(`true_false ${index + 1}: unrecognized correct answer.`);
            }

            const canonicalAnswer = parsedAnswer ? 'True' : 'False';
        ...
        if (q.type === 'ordering') {
            ...
            if (q.correctAnswers.length < 2) {
                ...
                if (!fallback) {
                    throw new Error(`ordering ${index + 1}: could not reconstruct the correct answer.`);
                }
        ...
        if (q.type === 'short_text') {
            ...
            if (!q.correctAnswers.length) {
                throw new Error(`short_text ${index + 1}: no reference answer.`);
            }
        }
    ...
    async function generateForCurrentVideo() {
        ...
        try {
            setStatus('Preparing Gemini Nano and the YouTube transcript...');
            ...
            const compactNote = cachedStudyChunks.length
                ? `; bank: ${cachedStudyChunks.length} fragments, ` +
                `current batch: ${quizTranscript.length} characters`
                : '';
            setStatus(
                `Transcript: ${transcript.length} characters${compactNote}. ` +
            ...
            try {
                ...
                setStatus('Internal Gemini Nano error. Retrying generation...');
                quiz = await generateQuizWithNano(quizTranscript, generationSession);
            }
            ...
            const repairNote = quiz._repairs
                ? ` Normalized questions: ${quiz._repairs}.`
                : '';
            const skippedNote = quiz._skipped
                ? ` Invalid questions skipped: ${quiz._skipped}.`
                : '';

            setStatus(
                `Created: ${quiz.basic.length} required + ` +
    ...
    async function generateMoreQuestions() {
        ...
        localStatus.textContent = 'Preparing more questions...';
        ...
        try {
            ...
            if (quiz._skipped) {
                ...
                note.textContent =
                    `Added ${quiz.basic.length + quiz.advanced.length}; ` +
                    `invalid questions skipped: ${quiz._skipped}.`;
        ...
        check.addEventListener('click', async () => {
            ...
            try {
                ...
                if (
                    ...
                    showFeedback(feedback, 'bad', 'Select or enter an answer first.');
                    return;
                }
                ...
                showFeedback(
                    feedback,
                    'bad',
                    `Answer-check error: ${error?.message || String(error)}`
                );
            } finally {
    ...
    function buildAnswerControl(host, question) {
        ...
        throw new Error(`No UI renderer for question type ${question.type}`);
    }
    ...
    async function checkShortText(question, userAnswer, feedback) {
        ...
        showFeedback(feedback, 'wait', 'Checking the answer locally...');
    ...
    async function semanticShortTextCheck(question, userAnswer) {
        ...
        const check = answerCheckQueue.then(async () => {
            ...
            if (normalizedResult === 'true') return true;
            if (normalizedResult === 'false') return false;

            throw new Error('Gemini Nano returned an unrecognized answer-check result.');
        });
    ...
    function updateProgress() {
        ...
        progress.textContent =
            `Required: ${basicCorrect}/${basicCards.length}   |   ` +
            `Optional: ${advancedCorrect}/${advancedCards.length}`;
    }
